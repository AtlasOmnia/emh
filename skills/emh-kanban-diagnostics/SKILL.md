...
- Queue counts disagree with dispatcher or gateway status, worker liveness, task runs, or worker logs.
...
5. Classify the task lifecycle: queued, claimed, dispatched, spawned, running, failed, or completed. Use task runs and bounded worker log evidence only for the named task.
6. Compare worker process/liveness, run outcome, heartbeat, and log timestamps. A stale log is not a current run result.
...
5. **Is the run running, failed, or completed while the task state disagrees?**
   - Yes: compare run outcome, heartbeat, event ordering, and bounded log timestamps.
...
The `runs` and `log` commands require a named task ID. Use a task identifier supplied by the operator, never a broad scan. Do not use `tail`, `watch`, or `dispatch` as a diagnostic shortcut.
...
Every proposed action must name scope, expected effect, a verified backup where state exists, rollback, abort condition, and post-change verification. No autonomous repair is permitted.
...
- **Pitfall: a stale worker log is current evidence.** Recovery: match log timestamps and task/run identity; retain only a bounded redacted excerpt.
...
- [ ] Queue/board, dispatcher/gateway, worker spawn, run, heartbeat, and log evidence are separate.
...
- [ ] Raw task content, private identifiers, credentials, and raw logs are absent from shared evidence.
...
- **Minimal evidence:** stats/list output, named task run/log summary, gateway status, bounded gateway log excerpt, and installed-source or official URL.

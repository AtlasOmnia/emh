...
Orientation is the first-run consent flow for EMH's two optional integrations:
...
Both crons are registered on the **operator's default profile**, not on the EMH profile, because delivery channels (Telegram, Discord, and similar) belong to the profile that owns the job, and profiles do not inherit each other's channels. current runtime and installed source outrank generic guidance; the official docs are authoritative current documentation, but qualify claims against the installed Hermes version, platform, and configured surface. Nothing is registered, changed, or updated without the operator's explicit consent, and each consent is recorded before any mutation.
...
- The operator wants to install or re-configure the optional crons on their default profile.
- A first-run check is needed to verify the operator's setup state before offering integrations.

**Don't use for:** triage of a reported Hermes failure (use `emh-triage`); diagnosing a broken gateway or channel (use `emh-gateway-diagnostics`); release/version comparison (use `emh-release-intelligence`); update readiness or rollback planning (use `emh-update-recovery`); running the nightly sweep itself (use `emh-nightly-self-check`); or any request to apply an update — orientation only checks and reports, and update application is a separate explicit action.
...
4. **Explain the repo update-check cron.** Summarize that it compares the installed distribution version against the recorded source's latest commit once a week, reports what changed (read-only), and never applies anything. State whether the source has a remote; without one, explain that update-checking is unavailable for this install and skip the offer.
5. **Ask for consent to register the update-check cron.** Record the answer even when declined.
6. **Register the chosen crons (approval-gated mutations), each followed by verification.** Register from the default-profile context (see Exact commands) so the jobs land in the default store with access to its channels.
7. **Re-check the default cron inventory** and confirm each registered job's name, schedule, deliver target, toolsets, and prompt fingerprint. Report what was installed, what was declined, and the residual caveats (for example a `local`-only deliver when no channel was detected).
...
5. **Did the operator consent to this specific integration?** — No: record the decline and move on. Yes: register, then verify.
6. **Did registration verify cleanly?** — No: do not claim success; escalate with the evidence. Yes: report and close.
...
Every mutation requires explicit approval, a verified backup when reversal is difficult, a rollback procedure (remove the job via `hermes cron remove <job-id>`), and post-change verification. Never silently: register crons, change an existing job, apply updates, touch another profile's store, or edit the EMH distribution files.
...
- [ ] Each integration was explained, then consent asked and recorded individually.
